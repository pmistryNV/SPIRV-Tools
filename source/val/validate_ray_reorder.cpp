...
spv_result_t ValidateHitObjectPointer(ValidationState_t& _,
  ...
  if (!type || type->opcode() != SpvOpTypeHitObjectNV) {
    return _.diag(SPV_ERROR_INVALID_DATA, inst)
...
spv_result_t ValidateHitObjectInstructionCommonParameters(
  ...
  if (isValidId(payload_index)) {
    ...
    if (!variable || var_opcode != SpvOpVariable ||
        (SpvStorageClass(variable->GetOperandAs<uint32_t>(2)) !=
  ...
  if (isValidId(hit_object_attr_index)) {
    ...
    if (!variable || var_opcode != SpvOpVariable ||
        SpvStorageClass(variable->GetOperandAs<uint32_t>(2)) !=
            SpvStorageClassHitObjectAttributeNV) {
      return _.diag(SPV_ERROR_INVALID_DATA, inst)
  ...
  auto RegisterOpcodeForValidModel = [](ValidationState_t& _,
    ...
    _.function(inst->function()->id())
        ->RegisterExecutionModelLimitation(
            [opcode_name](SpvExecutionModel model, std::string* message) {
